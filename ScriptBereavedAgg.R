...
# 1. Format pre-treatment covariates for the baseline balance table.
# Categorical and dummy variables are converted to R 'factor' objects to report counts (N) and (%).
# Ordinal/Continuous metrics (like SPHUS, ISCED Education, ADL, Max Grip) remain numeric to report Mean and SD.

table1_data <- df_imputed %>%
  mutate(
    # Create the grouping variable (this will be used for columns, NOT rows)
    Treatment = ifelse(is_treated == 1, "Palliative Care", "Standard Care"),
    
    # Convert numeric dummy to categorical factor for proper count display
    Gender = factor(gender_imp, levels = c(1, 2), labels = c("Male", "Female")),
    
    # Cause of Death (SHARE standard coding grouped for clinical clarity)
    ...
    # Collapse multiple binary diagnosis dummies into a single categorical factor.
    Diagnosis = case_when(
      has_cancer == 1 ~ "Cancer",
      has_neuro == 1 ~ "Neurological",
      has_organ == 1 ~ "Organ Failure",
      TRUE ~ "Other/None"
    ),
    Diagnosis = factor(Diagnosis, levels = c("Cancer", "Neurological", "Organ Failure", "Other/None"))
  ) %>%
  # Select ONLY the variables we want to display in the table, plus the grouping variable
  select(
    Treatment,
    `Age (Years)` = age_imp,
    Gender,
    `Education Level (ISCED 0-6)` = edu_imp, # Kept as continuous numeric
    `Primary Diagnosis` = Diagnosis,
    `Cause of Death`,
    `Self-Perceived Health (1-5)` = sphus_imp, # Kept as continuous numeric
...
# 2. Generate the balance summary as a 'gt' object
library(modelsummary)
library(gt)

baseline_table_gt <- datasummary_balance(
  ~ Treatment, # Treatment dictates the columns and is automatically removed from rows
  data = table1_data,
  fmt = 2, # Format to 2 decimal places
  output = "gt" # Directs modelsummary to return a high-quality gt table object
)

# 3. Apply academic formatting and export to PNG
...
# Export the table as a high-resolution PNG image for manuscript review
...
create_appendix_table <- function(model_list, title_text, filename) {
  ...
  gt_tab <- tab_data %>% 
    ...
    # AGGIUNTA: Rinomina le colonne per la visualizzazione
    cols_label(
      Model = "Subgroup",
      `t-3` = "t = -3",
      `t-2` = "t = -2",
    ...
    # Aggiungiamo anche un font accademico opzionale per renderla perfetta per il paper
...
if(nrow(stats_full) > 0 && nrow(stats_restr) > 0) {
  ...
  table_oster <- oster_results %>%
    ...
    # --- OTTIMIZZAZIONE DIMENSIONI E SPAZI ---
    tab_options(
      table.font.size = px(20),          # Font più grande (default è ~16)
      heading.title.font.size = px(22),  # Titolo più evidente
      column_labels.font.size = px(20),  # Intestazioni più grandi
      data_row.padding = px(4),          # Riduce lo spazio tra le righe (compattezza)
      table.width = px(800),             # Forza una larghezza fissa per evitare dispersioni
      container.width = px(800)
    ) %>%
    
    # Forza la larghezza delle singole colonne per "stringere" la tabella
    cols_width(
      Time ~ px(150),
      Coeff_Main ~ px(120),
      R2_Move ~ px(150),
  ...
  # Quando salvi, usa vwidth e vheight per "ritagliare" l'immagine intorno alla tabella
  gtsave(table_oster, 
         filename = file.path(PATH_APP, "Table_App_OsterBounds.png"),
         vwidth = 900,   # Larghezza del "mirino" per lo scatto
         vheight = 500)  # Altezza del "mirino" (regola in base a quante righe hai)
